...
- The four routes use the separate Transit Engine contract: `TRANSIT_CALC_BASE_URL` plus server-only `ASTROLOGYAPI_USER_ID`/`ASTROLOGYAPI_PASSWORD` Basic authentication. Existing Birth Chart, Horoscope, and Moon calls continue to use `ASTROLOGY_API_BASE_URL` and `X_ASTROLOGYAPI_KEY`.
